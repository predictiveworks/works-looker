...
object LookLogger {
  ...
  private var instance:Option[LookLogger] = None
  private var logger:Option[Logger] = None

  def getInstance(name:String,path:String):LookLogger = {
    if (instance.isEmpty) {
      val beatLogger = new LookLogger(name, path)
...
class LookLogger(name:String, path:String) {
